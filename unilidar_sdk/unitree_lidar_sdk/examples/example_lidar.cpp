...
#include <unistd.h> // For getpid(), fork(), kill
#include <csignal> // For kill()

using namespace unitree_lidar_sdk;

pid_t alarm_pid = -1; // Global variable to store the alarm process PID

void startAlarm() {
if (alarm_pid == -1) {
	alarm_pid = fork(); // Create a new process
	if (alarm_pid == 0) {
	// Child process: play the alarm
		execlp("aplay", "aplay", "-q", "/home/orangepi/Desktop/unilidar_sdk-main/unitree_lidar_sdk/examples/real_ikun.wav", NULL);
	// If execlp fails
		perror("Failed to play alarm");
		exit(1);
		}
	}
}
...
void stopAlarm() {
	if (alarm_pid > 0) {
		kill(alarm_pid, SIGKILL); // Terminate the alarm process
		alarm_pid = -1; // Reset the PID
	}
}

float spherer(float a, float b, float c) {
	float r = a * a + b * b + c * c;
	return r;
}
...
int main(){
  ...
  while (true)
  {
    ...
    case IMU:
	  /*if(spherer(lreader->getIMU().quaternion[0], lreader->getIMU().quaternion[1], lreader->getIMU().quaternion[2])<0.04 ) {
    ...
    case POINTCLOUD:
      ...
      for (int i = 0; i< 100; i++){ // print the first 10 points
        printf("\t  (%f, %f, %f, %f, %f, %d)\n", 
      ...
      printf("\ttimedelay (us) = %d\n\n", lreader->getTimeDelay()); 
      break;*/
      bool alert_inf = false;
      for (const auto& point : lreader->getCloud().points){
		  double radius = point.x * point.x + point.y * point.y + point.z * point.z;
		  if (radius <= 0.04){
			  alert_inf = true;
			  }
		  }
	  if (alert_inf){
		  printf("ALARM: Point detected within 20cm radius!\n");
			startAlarm(); // Start playing alarm
	  }else {
		stopAlarm();
	  }
	  break;
